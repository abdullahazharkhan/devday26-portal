...
import ManageAmbassadors from './ManageAmbassadors'

const tabs = teamConfig['ambassador-management'].tabs.map((t) => ({ id: t.action, title: t.label.toUpperCase() }))
...
export default async function AmbassadorManagementPage({ searchParams }: Props) {
    ...
    return (
        <div className="flex flex-col gap-8">
            ...
            {activeTab.id === 'view-ambassador-dashboard' ? (
                <AmbassadorDashboard />
            ) : activeTab.id === 'manage-ambassadors' ? (
                <ManageAmbassadors />
            ) : (
