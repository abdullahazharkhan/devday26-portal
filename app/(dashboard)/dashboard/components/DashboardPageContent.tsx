...
const NO_ACCESS = (
    <div className="border border-primaryred-muted bg-[#271C1C] p-5 sm:p-8 md:p-10 min-h-64 sm:min-h-80 flex items-center justify-center">
        <p className="text-[#C4C4C4] text-xs tracking-widest">NO_ACCESS_ASSIGNED</p>
    </div>
)
...
export default function DashboardPageContent({ tabParam }: Props) {
    ...
    const requestedTab = normalizeActionId(searchParams.get('tab') ?? tabParam ?? '')
    ...
    const activeActionId =
        requestedTab && userActionSet.has(requestedTab)
            ? requestedTab
            : (userActions[0] ?? '')

    const title =
        ALL_ACTIONS[activeActionId]?.label?.toUpperCase() ??
        (activeActionId ? activeActionId.toUpperCase() : 'DASHBOARD')
